...
import com.salesforce.androidsdk.rest.ClientManager.Companion.ACCESS_TOKEN_REFRESH_INTENT
import com.salesforce.androidsdk.rest.ClientManager.Companion.ACCESS_TOKEN_REVOKE_INTENT
import com.salesforce.androidsdk.rest.ClientManager.Companion.EXTRA_TOKEN_ERROR
import com.salesforce.androidsdk.rest.ClientManager.Companion.EXTRA_TOKEN_ERROR_DESCRIPTION
import com.salesforce.androidsdk.rest.ClientManager.Companion.INSTANCE_URL_UPDATE_INTENT
...
class ClientManagerMockTest {
    ...
    fun setUp() {
        ...
        mockSDKManager = mockk<SalesforceSDKManager> {
            ...
            every { registerUsedAppFeature(any(), any()) } returns Unit
    ...
    fun testGetNewAuthToken_RefreshTokenRotation_UpdatesCachedRefreshToken() {
        ...
        assertEquals(ROTATED_REFRESH_TOKEN, run { val f = authTokenProvider.javaClass.getDeclaredField("refreshToken"); f.isAccessible = true; f.get(authTokenProvider) as String? })
    }
    ...
    fun testGetNewAuthToken_RefreshTokenRotation_SubsequentRefreshSucceeds() {
        ...
        assertEquals(REFRESHED_ACCESS_TOKEN, authTokenProvider.getNewAuthToken())
        assertEquals(firstRotated, run { val f = authTokenProvider.javaClass.getDeclaredField("refreshToken"); f.isAccessible = true; f.get(authTokenProvider) as String? })
        assertEquals(firstRotated, persistedRefreshToken)

        // Second refresh, ensure each rotation is stored.
        assertEquals(REFRESHED_ACCESS_TOKEN, authTokenProvider.getNewAuthToken())
        assertEquals(secondRotated, run { val f = authTokenProvider.javaClass.getDeclaredField("refreshToken"); f.isAccessible = true; f.get(authTokenProvider) as String? })
        verify(exactly = 0) {
            mockSDKManager.logout(any(), any(), any(), any())
        }
    }
    ...
    fun testGetNewAuthToken_ClientBlockedRetry_SubsequentCallSkipsInvalidateToken() {
        val result = setupTokenErrorScenario("client_blocked_retry", "Attestation verification pending")
        val clientManagerSpy = spyk(clientManager)
        every { clientManagerSpy.getAccounts() } returns arrayOf(result.mockAccount)
    ...
    fun testGetNewAuthToken_StorageAdvanced_AdoptsWithoutRefreshing() {
        ...
        assertEquals(newRefreshToken, run { val f = provider.javaClass.getDeclaredField("refreshToken"); f.isAccessible = true; f.get(provider) as String? })
        verify(exactly = 0) { mockSDKManager.logout(any(), any(), any(), any()) }
    ...
    fun testGetNewAuthToken_ConcurrentBurst_NullInstanceUrlLoser_KeepsOwnInstanceUrl() {
        ...
        assertEquals("https://loser.instance.url", loser.getInstanceUrl())
        verify(exactly = 0) { mockSDKManager.logout(any(), any(), any(), any()) }
    }
